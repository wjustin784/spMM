...
static void write_f32_list(const std::string& path, const std::vector<float>& v){
    std::ofstream f(path);
    for(size_t i=0;i<v.size();++i){ f.setf(std::ios::fmtflags(0), std::ios::floatfield);
        f.precision(9); f<<v[i]<<'\n'; }
}
...
static float get_chunk_fraction(int argc, char** argv){

    float cf = 0.2f;
    if(argc>=5){
        cf = std::strtof(argv[4], nullptr);
    }else{
        const char* s = std::getenv("CHUNK_FRACTION");
        if(s) cf = std::strtof(s, nullptr);
    }
    if(cf <= 0.0f || cf > 1.0f){
        std::fprintf(stderr,"chunk_fraction must be in (0,1], got %f\n", cf);
        std::exit(1);
    }
    return cf;
}
...
int main(int argc, char** argv){
    if(argc<4){
        std::fprintf(stderr,"Usage: %s A_prefix B_prefix C_prefix [chunk_fraction]\n", argv[0]);
        return 2;
    }
    std::string Apre=argv[1], Bpre=argv[2], Cpre=argv[3];
    const float chunk_fraction = get_chunk_fraction(argc, argv);
    ...
    cusparseSpMatDescr_t dAmat,dBmat,dCmat;
    CHECK_CUSPARSE(cusparseCreateCsr(&dAmat, A.rows, A.cols, A.nnz,
        dAptr,dAind,dAval, CUSPARSE_INDEX_32I,CUSPARSE_INDEX_32I,
        CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F));
    CHECK_CUSPARSE(cusparseCreateCsr(&dBmat, B.rows, B.cols, B.nnz,
        dBptr,dBind,dBval, CUSPARSE_INDEX_32I,CUSPARSE_INDEX_32I,
        CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F));

    CHECK_CUDA(cudaMalloc(&dCptr, (A.rows+1)*sizeof(int32_t)));
    CHECK_CUSPARSE(cusparseCreateCsr(&dCmat, A.rows, B.cols, 0,
    ...
    size_t buf1=0; void *dBuf1=nullptr;
    CHECK_CUSPARSE(cusparseSpGEMM_workEstimation(h,opA,opB,&alpha,dAmat,dBmat,&beta,dCmat,
    ...
    int64_t num_prods=0;
    CHECK_CUSPARSE(cusparseSpGEMM_getNumProducts(desc, &num_prods));
    (void)num_prods; 

    size_t buf3=0, buf2=0; void *dBuf3=nullptr, *dBuf2=nullptr;
    CHECK_CUSPARSE(cusparseSpGEMM_estimateMemory(h, opA, opB,
        &alpha, dAmat, dBmat, &beta, dCmat, computeType, alg,
        desc, chunk_fraction, &buf3, nullptr, nullptr));
    if(buf3>0) CHECK_CUDA(cudaMalloc(&dBuf3, buf3));
    CHECK_CUSPARSE(cusparseSpGEMM_estimateMemory(h, opA, opB,
        &alpha, dAmat, dBmat, &beta, dCmat, computeType, alg,
        desc, chunk_fraction, &buf3, dBuf3, &buf2));
    if(dBuf3) CHECK_CUDA(cudaFree(dBuf3)); 

    if(buf2>0) CHECK_CUDA(cudaMalloc(&dBuf2, buf2));


    CHECK_CUSPARSE(cusparseSpGEMM_compute(h,opA,opB,&alpha,dAmat,dBmat,&beta,dCmat,
        computeType,alg,desc,&buf2,dBuf2));


    int64_t Cr,Cc,Cnnz; CHECK_CUSPARSE(cusparseSpMatGetSize(dCmat,&Cr,&Cc,&Cnnz));
    CHECK_CUDA(cudaMalloc(&dCind, Cnnz*sizeof(int32_t)));
    CHECK_CUDA(cudaMalloc(&dCval, Cnnz*sizeof(float)));
    CHECK_CUSPARSE(cusparseCsrSetPointers(dCmat, dCptr, dCind, dCval));


    CHECK_CUSPARSE(cusparseSpGEMM_copy(h,opA,opB,&alpha,dAmat,dBmat,&beta,dCmat,
    ...
    std::cout<<"[C++] ALG3 wrote "<<Cpre
             <<"_* (rows="<<C.rows<<", cols="<<C.cols
             <<", nnz="<<C.nnz<<", chunk_fraction="<<chunk_fraction<<")\n";
    ...
    cusparseDestroySpMat(dAmat); cusparseDestroySpMat(dBmat); cusparseDestroySpMat(dCmat);
    cusparseDestroy(h);
    return 0;
}
